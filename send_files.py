...
redis_client = redis.StrictRedis(
    host=constant.REDIS_HOST_IP, port=constant.REDIS_HOST_PORT, db=0
)
...
def classifyFiles(filename):
    ...
    if file_part == "a":
        ...
        first_file_name = os.path.basename(
            redis_client.get(filename[:-1] + "ab").decode("utf-8")
        )
...
def send_http_request(first_file_name, filename, files_to_send):
    response = requests.post(
        "http://{ip}:{port}/merge_and_sign".format(
            ip=constant.HAPROXY_SERVER_IP, port=constant.HAPROXY_SERVER_PORT
        ),
        files=files_to_send,
    )
