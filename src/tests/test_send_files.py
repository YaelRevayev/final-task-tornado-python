...
class TestSendFiles(unittest.TestCase):
    ...
    @patch("send_files.error_or_success_logger", return_value=MagicMock())
    def test_classify_files_no_key_exists(
        self, mock_error_or_success_logger, mock_remove_extension, mock_get_storage
    ):
        ...
        mock_error_or_success_logger.debug.assert_any_call(
            "opened new process for image100_a.jpg"
        )
        mock_error_or_success_logger.debug.assert_any_call("no key exists")
    ...
    def test_classify_files_key_exists(
        ...
        mock_storage.get.return_value = "first_file_name"
        mock_get_storage.return_value = mock_storage
        ...
        self.assertTrue(mock_error_or_success_logger.debug.called)
    ...
    def test_handle_existing_key(
        self, mock_remove_file_from_os, mock_error_or_success_logger, mock_send_files
    ):
        mock_storage = MagicMock()
        ...
        mock_error_or_success_logger.debug.assert_called_once_with("key does exists")
